...
allfilelist <- list.files(path = "/path/data/VCF/SMC/" 
                          pattern = "germline", recursive = TRUE, full.names = TRUE)


process_file <- function(filepath) {
  read.delim(filepath) %>%
    mutate(patient_id = str_sub(filepath, 75, 83)) %>%
    filter(Chr != "chrY", DP >= 10, GQ >= 50, VAF >= 0.3, FILTER == "PASS") %>% # SMC
    #filter(Chr != "chrY", DP >= 15, GQ >= 50, VAF >= 0.2, FILTER == "PASS") %>% # TARGET
    filter(!str_detect(ExonicFunc, "exonic") | str_detect(Func, "splicing")) %>%
}

raw_variant <- map(allfilelist, process_file) %>% bind_rows()
...
```{r}
raw_variant_smc <- raw_variant_smc %>%
  filter(KRGDB1100 < 0.01 | is.na(KRGDB1100)) %>%
  filter(ExAC < 0.01 | is.na(ExAC)) %>%
  ...
  distinct(germline_id, Gene, Exon, .keep_all = TRUE) %>%
  ...
  mutate(unique_ids = n_distinct(germline_id)) %>%
...
write.table(error_mat, file="/Users/eunseopseo/Dropbox/Project/NBL/Germline/results_final/cindex_500_TR.txt", sep="\t", row.names = TRUE, col.names=TRUE, quote = FALSE)
```
...
write.table(error_mat_te, file="/Users/eunseopseo/Dropbox/Project/NBL/Germline/results_final/cindex_500_TE.txt", sep="\t", row.names = TRUE, col.names=TRUE, quote = FALSE)
```
